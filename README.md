...
Each ray on the PCB is a copper pour used to spread and hold heat from the resistors. It was designed using Python and an SVG drawing library, and converted into a KiCad footprint (see `noteboks/display.ipynb`).

![front face of clock](assets/display_front_sm.jpeg)
![front face of clock](assets/display_back_sm.jpeg)

The control board is driven by an Atmega328PB and uses a DS3231MZ RTC for accurate time keeping. A coin cell battery is used to keep track of time even when the clock is unpowered. The time is initially set when programming the clock, and can changed in +/- 30 minute increments using the two buttons on the back.
